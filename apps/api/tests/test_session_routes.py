...
def test_submit_vector_returns_ok():
    ...
    resp = client.post(f'/sessions/{session_id}/submit', json={'participant_id': pid, 'vector': LAGER_VECTOR})
    assert resp.status_code == 200
...
def test_status_shows_completed_count():
    client, _ = make_client()
    session_id = client.post('/sessions', json={'host_id': 'h1'}).json()['session_id']
    pid = client.post(f'/sessions/{session_id}/join', json={'name': 'Alice'}).json()['participant_id']
    client.post(f'/sessions/{session_id}/join', json={'name': 'Bob'})
    client.post(f'/sessions/{session_id}/submit', json={'participant_id': pid, 'vector': LAGER_VECTOR})
