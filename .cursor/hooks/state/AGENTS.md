...
- Do not show developer-facing tips or algorithmic debug details (e.g. α/β score breakdown) in primary user UI.
- Follow the repo's 7-phase Matt Pocock skill pipeline for feature work (grill-with-docs → to-prd → to-issues → tdd).
- Home "Start a session" should use inline quick selection (vibe + ABV) that starts a real session, not only link elsewhere.
...
- Production domain is beerolog.com.
- Clerk handles authentication.
- Railway hosts the API; monorepo layout requires explicit start/build configuration.
...
- Catalog seed data must not persist Untappd references (URLs, IDs, ratings, or aggregator provenance).
- Beer catalog images are hosted on Vercel Blob; persisted `image_url` values must use blob storage, not third-party CDNs.
