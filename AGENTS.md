...
- Production stack: Vercel hosts apps/web (`beerolog`, TanStack Start SSR) and apps/api (`beerolog-api`, FastAPI via uv); Neon Postgres is the database; Clerk handles authentication. Domains: beerolog.com / api.beerolog.com (defaults beerolog.vercel.app / beerolog-api.vercel.app). Web mounts Vercel Analytics + PostHog in the root layout. PostHog runs the FULL suite (autocapture, $pageview, session replay [input-masked], feature flags, exception tracking, persistent cookie identity — see `docs/services/posthog.md`), active once `VITE_POSTHOG_PROJECT_TOKEN` is set (all Vercel envs). ⚠ It sets analytics cookies + records sessions WITHOUT opt-in consent — known compliance debt, gated work in `docs/legal/legal-launch-followups.md` before public launch.
