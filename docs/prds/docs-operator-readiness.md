...
3. Configure web, API, Cognito, Neon, Railway, Vercel, and OpenAI correctly for preview and production-like environments.
...
6. As an operator, I want the Cognito callback URLs, sign-out URLs, Vercel origins, and API CORS origins documented as one aligned contract, so that sign-in works reliably after deploy.
...
11. As a support person, I want common failure modes documented, so that I can quickly narrow issues like bad Cognito redirects, CORS drift, missing migrations, unsafe secrets, or unavailable provider configuration.
12. As a maintainer, I want the documentation to explain which behaviors depend on Neon, Cognito, Railway, Vercel, and OpenAI, so that I can reason about blast radius when one provider is misconfigured.
...
- Operator docs must describe origin alignment as one contract across Vercel, Railway, and Cognito. Supported web origins, API CORS allowlist values, and Cognito callback/sign-out URLs should be documented together so drift is easier to detect.
...
- Replacing Vercel, Railway, Cognito, Neon, or OpenAI with different providers
